...
class PasswordDialog(Dialog):
    ...
    def body(self, master):
        message_label = Label(
            master, text="Enter password to access chat room.")
        password_entry = Entry(master, width=27, bd=1, show="\u2022", relief=SOLID, font=(
            "Consolas", 10))  # show="" to reset
    ...
    def buttonbox(self):
        ok_btn = Button(self, text="OK", width=7,
...
class ColourDialog(Dialog):
    ...
    def body(self, master):
        ...
        for colour in self.colours:
            colour_btn = Button(master, bg=colour, width=2,
                                font=("Consolas", 6), padx=5, pady=5)
...
class DrawDialog(Dialog):
    ...
    def draw_dot(self, event):
        ...
        oval_id = self.canvas.create_oval(
            x1, y1, x2, y2, fill=fill, outline=fill)
        # Store data for actions.
        self.new_action.append({"type": "oval", "id": oval_id, "coords": [
    ...
    def draw_line(self, event):
        ...
        line_id = self.canvas.create_line(
            (x1, y1, x2, y2), fill=fill, width=self.brush_size, capstyle=ROUND)
        # Store data for actions.
        self.new_action.append({"type": "line", "id": line_id, "coords": [
                               x1, y1, x2, y2], "width": self.brush_size, "fill": fill})
    ...
    def redo(self):
        ...
        self.current_actions.append(
            self.all_actions[len(self.current_actions)].copy())
        for a in self.current_actions[-1]:
            # Re-create line or oval on canvas, setting new shape id.
            if a["type"] == "line":
                action_id = self.canvas.create_line(
                    a["coords"], fill=a["fill"], width=a["width"], capstyle=ROUND)
            else:  # a["type"] == "oval"
                action_id = self.canvas.create_oval(
                    a["coords"], fill=a["fill"], outline=a["fill"])
            a["id"] = action_id
        # Disable/enable buttons
        self.undo_btn.config(state=NORMAL)
    ...
    def buttonbox(self):
        ...
        self.colour_btn = Button(button_frame, bg=self.brush_colour,
                                 bd=3, font=SMALL_FONT, width=7, command=self.change_colour)
        self.eraser_btn = Button(button_frame, text="Eraser", bd=3,
                                 font=SMALL_FONT, width=7, command=self.toggle_eraser)
        self.size_up_btn = Button(button_frame, text="Size +", bd=3,
                                  font=SMALL_FONT, width=7, command=lambda: self.change_size(1))
        self.brush_size_canvas = Canvas(
            button_frame, width=73, height=73, bg="white")
        self.brush_size_canvas.future_width = 77
        self.brush_size_canvas.future_height = 77
        self.size_down_btn = Button(button_frame, text="Size -", bd=3,
                                    font=SMALL_FONT, width=7, command=lambda: self.change_size(-1))
        self.undo_btn = Button(button_frame, text="Undo", bd=3,
                               font=SMALL_FONT, width=7, command=self.undo, state=DISABLED)
        self.redo_btn = Button(button_frame, text="Redo", bd=3,
                               font=SMALL_FONT, width=7, command=self.redo, state=DISABLED)
        self.clear_btn = Button(button_frame, text="Clear", bd=3,
                                font=SMALL_FONT, width=7, command=self.clear_canvas)
        self.send_btn = Button(button_frame, text="Send", bd=3,
                               font=SMALL_FONT, width=7, command=self.ok, default=ACTIVE)
        self.close_btn = Button(
            button_frame, text="Close", bd=3, font=SMALL_FONT, width=7, command=self.cancel)
    ...
    def apply(self):
        if self.validate():
            ...
            self.result = (self.grab_image(), {
                           "brush_size": self.brush_size, "brush_colour": self.brush_colour})
        return True
